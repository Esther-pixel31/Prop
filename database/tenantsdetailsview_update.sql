CREATE OR REPLACE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `tenantsdetailsview` AS 
SELECT DISTINCT 
...
FROM 
    ...
    LEFT JOIN `invoices` ON (`tenants`.`tenantID` = `invoices`.`tenantID`));
